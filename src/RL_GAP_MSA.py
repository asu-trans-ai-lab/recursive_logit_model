...
def MAXUility(link_flow_network,link_sys_utility_network,network):
    ...
    for demand in g_demand_list:
        ...
        for node in node_NoD_list:
            ...
            for to_node in g_nodes_list[node_ListNo].outgoing_node_list:
                link=(node,to_node)
                sum_utility_pre+=math.exp(1/mu*link_sys_utility[network_cur.index(link)])
            
            
            Pi[node_list.index(node)]=math.log(sum_flow_pre/sum_utility_pre) 
...
if __name__=='__main__':
    ...
    ax.plot(range(g_outer_iteration_No-1),gap)
